...
from src.utils.metrics import json_serialize, f1_score, ece_score, bald_score
...
def preprocess_test_data(df: pd.DataFrame) -> tf.data.Dataset:
    ...
    dataset = tf.data.Dataset.from_tensor_slices((
        ...
        labels
    )).batch(16).cache().prefetch(tf.data.experimental.AUTOTUNE)
    return dataset
...
def main(args):
    ...
    subset_size = 100
    input_dataset.test = input_dataset.test.sample(n=min(subset_size, len(input_dataset.test)))
