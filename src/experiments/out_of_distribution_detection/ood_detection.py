...
def bert_teacher_mc_dropout(model, eval_data, n=50) -> Results:
    ...
    return Results(y_true, y_pred_mcd, y_prob_mcd, var_mcd, acc, prec, rec, f1, auc, nll, bs, unc, res, rel, ece, ece_l1, bald)
...
def bert_student_monte_carlo(model, eval_data, n=50):
    ...
    return Results(y_true, y_pred_mc, y_prob_mc, var_mc, acc, prec, rec, f1, auc, nll, bs, unc, res, rel, ece, ece_l1, bald)
...
def main(args):
    ...
    input_dataset = Dataset(test=load_data(args.input_data_dir, 'train'))
    data_tf = preprocess_data_bert(input_dataset.train)
    ...
    if args.run_for_augmented_student:
        ...
        results_bert_augmented_student = perform_experiment_bert_student(augmented_student, data_tf, n_trials=args.n_trials)
